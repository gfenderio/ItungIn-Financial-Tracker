...
const Profile = () => {
    ...
    const userName = isGuest ? 'Guest' : (userProfile?.name || (language === 'id' ? 'Pengguna' : 'User'));
    const userEmail = isGuest ? '' : (userProfile?.email || 'user@example.com');
                        ...
                        <div className="bg-primary/5 dark:bg-primary/10 border border-primary/20 dark:border-primary/30 rounded-xl p-4 mt-2">
                            <div className="flex items-start gap-3 mb-3">
                                ...
                                <p className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                                    {language === 'id' 
                                        ? 'Hubungkan akun Google Anda untuk mengaktifkan sinkronisasi awan, fitur Ekspor/Impor, dan melindungi data Anda.'
                                        : 'Connect your Google account to enable cloud sync, Export/Import features, and protect your data.'}
                                </p>
                            </div>
                            <button
                                onClick={() => {
                                    updateUserProfile({ name: "Google User", email: "google.user@example.com", isGuest: false });
                                    // optional: showAlert depends on whether imported, using native alert or CustomAlert is preferred
                                    alert(language === 'id' ? 'Berhasil terhubung dengan Google (Mock)!' : 'Successfully connected to Google (Mock)!');
                                    setIsEditingProfile(false);
                                }}
                                className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold py-2.5 px-4 rounded-xl flex items-center justify-center gap-3 transition-colors shadow-sm text-sm"
                            >
                                ...
                                {language === 'id' ? 'Hubungkan Akun Google' : 'Connect Google Account'}
                            </button>
                        </div>
                    </div>
                ) : !isEditingProfile ? (
